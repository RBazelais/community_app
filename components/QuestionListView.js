...
//ImerMixins are a safeguard to prevent exceptions from being thrown
...
//"class QuestionListView extends React.Component {"" cannot be used will throw error
//can only be used with "React.createClass" because es6 deprecated it somehow...wtf
var QuestionListView = React.createClass({
 ...
 mixins: [TimerMixin],
   ...
   componentDidMount: function () {
     this.searchMedia('food');
     //default questions that will be displayed in the ListView
   },
   ...
   _urlForQuery: function (query: string): string{
     if (query.length > 2){
       return API_URL + 'APIquerigoeshere' + encodeURIComponent(query);
     }
   },
